...
export function SkillGapList({ gaps }: { gaps: SkillGap[] }) {
  const active = gaps.filter(gap => gap.expected_gap > 0 || gap.expected_level > gap.current_level);
  return (
    <section className="card">
      <div className="sectionTitle"><div><div className="eyebrow">КАРТА НАВЫКОВ</div><h2>Что отделяет вас от цели</h2></div><span className="pill">{active.filter(g => g.expected_gap > 0).length} gaps</span></div>
      ...
      <div className="gapList">
        {active.length ? active.map(gap => (
          <article className="gapRow" key={gap.skill_id}>
            ...
            <Dots value={gap.current_level} expected={gap.expected_level} required={gap.required_level} />
            <b>{gap.current_level} → {gap.required_level}</b>
          </article>
