...
import { CardAction, AgentReply, AppConfig, Artifact, Career, Employee, HrSummary, Recommendation, Session, View } from "../lib/types";
import { QuestIcon as Icon } from "./QuestIcon";
import { EmployeeView, TeamView } from "./QuestViews";

const labels: Record<View,string> = { comparison:"Сравнение", explanation:"Объяснение", clarification:"Уточнение", level:"Мой уровень", skill_map:"Карта навыков", route:"Мой маршрут", next_step:"Следующий шаг", history:"Моя история", hr_gaps:"Пульс команды", hr_coverage:"Без следующего шага", hr_events:"Участие", skill_guide:"Мастерская навыка" };
...
function Dashboard({session,config,profiles,switching,sessionError,changeSession,logout}: {session:Session;config:AppConfig;profiles:Employee[];switching:boolean;sessionError:string;changeSession:(id:string,role:Session["role"])=>Promise<void>;logout:()=>Promise<void>}) {
  ...
  function closeChat(){setChatOpen(false);requestAnimationFrame(()=>openButton.current?.focus());}
  ...
  async function complete(){
    ...
    try{const response=await client.complete(session.employee_id,confirmation.event_id,confirmation.participation_id);if(!alive.current)return;setCareer(response.career);setArtifact(null);setLastReply(null);setConfirmation(null);const text=response.already_completed?"Это участие уже учтено. Повторный прирост не начислен.":"Участие сохранено. Ожидаемый прогресс обновлён; подтверждённый уровень и грейд не меняются.";setNotice(text);setMessages(old=>[...old.map(m=>({...m,stale:!!m.reply})),{role:"assistant",text,reply:{answer:text,artifact:{kind:"skill_map",career:response.career},sources:[],trace:[],mode:"local",notice:"",elapsed_ms:0,constraints:{max_minutes:null,event_format:null},conversation_id:conversation.current||"",message_id:crypto.randomUUID()}}]);}
  ...
  const questions=hr?["Какие навыки проседают в команде?","У кого нет следующего шага?","Какое участие по активностям?"]:["Покажи карту навыков","Построй маршрут","У меня 30 минут"];
  const activeOptions=artifact?.options;
  return <div className={`quest-app ${chatOpen?"chat-open":""} ${menuOpen?"menu-open":""}`}>
    ...
    <section className="agent-panel" aria-label="Карьерный агент" aria-hidden={!chatOpen}>
      <div className="agent-heading"><div className="agent-symbol"><Icon name="spark" size={22}/></div><div><strong>Quest Agent</strong><span><i className={config.ai_enabled?"live":""}/>{config.ai_enabled?"AI-агент с инструментами":"Локальный помощник"}</span></div><button className="close-chat" aria-label="Свернуть чат" onClick={closeChat}><Icon name="close"/></button></div>
