...
export type View = "level" | "skill_map" | "route" | "next_step" | "history" | "hr_gaps" | "hr_coverage" | "hr_events" | "skill_guide";
...
export type Artifact = { kind: View; career?: Career; hr?: HrSummary; options?: Options | null; simulation?: { event_id: string; title: string; changes: { skill_id: string; before: number; after: number }[] }[] | null; guide?: { skill: SkillGap; example: string; label: string } | null };
export type AgentReply = { answer: string; artifact: Artifact | null; sources: { id: string; label: string; view: View }[]; trace: { tool: string; status: string; ms: number }[]; mode: "agent" | "local" | "fallback"; notice: string; elapsed_ms: number; constraints: { max_minutes: number | null; event_format: string | null } };
